...
> ⚠️ **GPT 리뷰는 옵트인이다**: 설계 문서를 PR로 올리는 것만으로는 리뷰가 돌지 않는다. **사용자가
> 명시적으로 "GPT 리뷰를 해달라"고 요청했을 때만** PR에 `/gpt-review` 코멘트를 달아 리뷰를 트리거한다.
> 사용자가 리뷰를 요청하지 않았으면 코멘트를 달지 말 것(문서 PR은 그냥 열어두거나 머지).

사용자가 **"GPT 리뷰"/"설계 리뷰 해줘"** 를 **명시적으로** 요청하면 — **네가 직접 리뷰하지 말고** 아래대로 한다:

1. 리뷰 대상 설계를 `docs/ai-workflow/`(architecture.md·requirements.md·adr/ 등)에 작성·갱신한다.
2. 새 브랜치로 커밋·푸시한다.
3. base=main으로 설계 PR을 연다(아직 리뷰는 안 돎).
4. PR에 **`/gpt-review` 코멘트를 단다** — 이 코멘트가 리뷰 트리거다.
   → `.github/workflows/gpt-design-review.yml`(채널 B)가 GPT 독립 리뷰를 PR 코멘트로 단다
   (repo Secret `OPENAI_API_KEY` 필요 — 없으면 무해 skip).
   → **재리뷰**가 필요하면 `/gpt-review` 코멘트를 한 번 더 단다.
5. 리뷰가 달리면 항목별 수용/기각을 판단·반영하고 `docs/ai-workflow/reviews/`에 처리 기록을 남긴다.

핵심: **"명시적 리뷰 요청 → `/gpt-review` 코멘트"** 이며, 리뷰는 GitHub Action이 GPT를 호출해 수행한다(에이전트가 직접 하지 않음). 워크플로는 이미 이 저장소에 설정돼 있으니 새로 만들지 않는다.
