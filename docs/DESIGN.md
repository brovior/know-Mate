...
**DRM 유휴 임계 — 장시간 방치 시 DRM 의심 문서 스킵 (`collector.drm_idle_threshold_sec`)**

`IdleScheduler._on_idle`이 트리거 시 측정한 실제 유휴 경과초를 콜백에 전달하고,
`CollectorWorker.set_idle_context(idle_elapsed_sec)`로 다음 `run()` 사이클 1회에만 적용한다
(1회성 소비 — 이후 수동 트리거는 기본값 0.0 "방금 활동함"으로 복귀). 유휴 경과가
`collector.drm_idle_threshold_sec`(기본 480초) 이상이면 DRM/SSO 세션이 만료됐을 가능성이
크다고 보고, `scheduler._is_drm_suspected(path)`가 True인 파일만 그 사이클의 생산자 단계에서
큐잉을 건너뛴다(state 불변 → 다음 유효 사이클에 자동 재시도, 로그·요약에 "유휴로 DRM 문서
스킵 N건" 표시). 정상 레거시 바이너리(OLE2 매직)나 정상 OOXML(zip 매직)은 COM을 타더라도
DRM 세션과 무관하게 열리므로 스킵 대상이 **아니다** — DRM 래핑으로 의심되는(두 시그니처 중
어느 것도 아닌) 파일만 대상이다. 수동 재인덱싱([지금 재인덱싱])·온보딩 최초 스캔은
`set_idle_context`를 호출하지 않으므로 이 로직에 전혀 관여하지 않는다.
...
`idle_elapsed=0.0`으로 즉시 1회 트리거한다 — 이 값은 DRM 스킵 임계보다 항상 작으므로 정상
사이클로 진행되어 밀린 DRM 문서가 자연히 캐치업된다(다음 정기 유휴 사이클, 최대 `idle_seconds`
뒤까지 기다리지 않음). `is_busy()`인 동안은 판정을 보류하고 다음 폴링에서 재시도한다.
