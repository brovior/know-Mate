...
요청만 하고 즉시 반환하는데(실제 종료까지 임시파일·애드인 정리 등으로 수 초 소요), 반환 직후
프로세스를 조회하면 스스로 정리 중인 Office가 거의 항상 "안 죽음"으로 잡혀 매번 강제종료된다
(레이스). 강제종료는 그 자체로 Resiliency 표식을 남기므로, 이 레이스가 관측된 반복 정리 로그의
직접 원인이었다.
...
**두 가지 다른 원인이 같은 증상(강제종료)으로 나타날 수 있다는 점을 설계 단계에서 인지했다** —
① 레이스(유예로 해결) vs ② 파이썬 쪽 COM 참조 잔존(유예를 줘도 매번 소진). 이 수정 하나로
①은 해결되지만 ②라면 개악(사이클마다 최대 grace_sec만큼만 느려짐)이므로, 로그에 실측 대기
시간을 반드시 남긴다: `Office 정상 종료 확인: N개 (대기 X초)` vs `유예 5초 초과 — 강제 종료`.
**배포 후 후자가 반복되면 이 수정은 미완료로 간주하고 COM 참조 누수 추적(2차 작업)으로
넘어간다** — "고쳤다"가 아니라 "로그로 원인을 가려낼 수 있게 했다"가 이번 수정의 실제 성과다.

`grace_sec=0`이면 대기 없이 기존 동작과 동일(비상 스위치). 워치독(`terminate_stuck_office`,
이미 멈춰 응답 없는 상황)은 이 유예와 무관 — 애초에 기다릴 이유가 없어 그대로 즉시 종료한다.
