...
| 상태 | 날짜 | 결정자 | 리뷰 |
|---|---|---|---|
| Accepted | 2026-07-24 | Claude (Chief Architect) | reviews/REVIEW-20260724-adr-0001-explicit-quit-for-tray-app-adr-0002-purge-projectio-*.md (1~13차, Blocker/Major 전건 처리·구현 반영, 리뷰11 M-1 축소채택·리뷰12 marker 비동기화/단일인스턴스 순서 정정·리뷰13 marker 해제 위치를 app.exec() 정상 반환 후로 재이동 근거 명시) |
...
대량삭제 차단과 동일하게 동일 op_sig에서 장기 억제되고(30분 백오프 반복 없음) 1회 UI 알림으로
업데이트를 안내한다(리뷰11 M-2로 "failed"와 분리; 호환 전체-로드 모드를 두지 않는 원칙은 유지 —
조용한 폴백 금지).
...
- 감수하는 것: 외부 요인으로 DB에만 남은 고아 경로의 복구가 지연될 수 있다 — **구성·파일
  ...
  정합화 — 24h 상한이 있어 무기한 방치는 아니다). projection API는 lancedb 0.34.0에서 실측
