...
- **호출 규약 비호환(메서드는 있지만 인자·반환 형태가 다른 lancedb 버전)은 서킷 브레이커
  없이 문서·배포 정책으로 대응한다**(설계 리뷰 19차 M-2 처리 — 이 위험은 인정하지만,
  N회 연속 동일 실패를 세는 회로 차단기를 도입하는 대신 더 단순한 두 가지로 충분하다고
  판단): ① `requirements.txt`의 `lancedb`를 실측 검증 버전으로 **정확히 고정**(`==0.34.0`).
  범위(`>=0.34.0,<0.35`)를 허용하면 실측하지 않은 0.34.x가 빌드에 선택될 수 있어
  "검증된 버전을 번들한다"는 목적 자체가 성립하지 않는다(리뷰20 M-1로 범위 → 정확
  고정으로 정정). 상위 버전으로 올릴 때는 projection 실측을 다시 하고 이 값과
  `lancedb_compat.SUPPORTED_VERSION`을 함께 갱신한다. ② 앱 시작 시 `knowmate.lancedb_compat.check_lancedb_version()`이 번들된
  버전을 이 범위와 대조해, 벗어나면 로그 ERROR + 트레이 알림(앱은 계속 실행). 배포가
  PyInstaller onedir(빌드 시점에 exe에 의존성 고정, 사용자가 버전을 바꿀 수 없음)이므로
  이 검사는 "런타임에 나쁜 버전이 설치되는 것을 막는" 서킷 브레이커가 아니라, **빌드
  환경에서 상한을 넘는 lancedb가 실수로 번들됐을 때 조기에 드러내는 진단 신호**다.
  런타임에 호출 규약이 실제로 어긋나면(이 진단을 통과했더라도) `_purge_removed_folders`의
  좁은 capability probe(리뷰16 M-3)가 `select(...).to_arrow()` 호출 예외를 여전히
  `"failed"`(30분 백오프)로 잡아낸다 — 무기한 재시도이긴 하지만, 번들 배포 특성상 이
  상태에 실제로 도달하려면 빌드 실수가 선행돼야 하므로(위 ①·② 검사가 조기에 잡음)
  잔존 위험은 낮다고 판단.
