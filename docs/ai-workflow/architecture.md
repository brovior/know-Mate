...
- **성능 수용 — 필수(리뷰5 m-2, 리뷰11 m-1로 승격, 리뷰12 m-2로 기준 구체화)**: 결과 스키마에
  벡터/원문이 없다는 사실과 약 6배 속도 향상은 강한 정황이지만, projection이 저장소 스캔
  단계에서 실제로 컬럼을 건너뛴다는 직접 증거는 아니다(리뷰11 m-1 지적) — 배포 전 아래
  절차로 **필수** 확보한다:
  - 테스트 DB: 10만 행, file_path는 사내 실사용 경로 분포를 대표하는 길이(30~200자 구간에서
    표본화)로 생성.
  - 측정: 프로세스 warm-up(테스트 DB 1회 open + 쿼리 1회 실행 후 GC)한 뒤, 강제
    reconciliation(프로젝션 쿼리 1회 전체 스캔)을 **5회 반복** 실행. 매 회 실행 직전·직후
    프로세스 peak RSS(OS 레벨, 예: `resource.getrusage(RUSSELF).ru_maxrss` 또는 작업관리자)를
    기록해 "직후 - 직전" 추가 할당량을 구한다.
    - 5회 측정값의 **중앙값이 30MB 이하**이면 합격(NFR-1 상한 충족).
  - 가능하면 LanceDB의 query plan/스토리지 read 통계도 함께 기록해 컬럼 프루닝을 직접
    근거로 남긴다(합격 판정의 필수 조건은 아님 — 보조 근거).
