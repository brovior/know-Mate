...
### 남은 확인 사항
1. **승격 해석 확정(사용자)** — §5의 정책표는 "3회 = 조치 필요"를 *상태*로 해석했다.
   *대기 시간*을 3회차부터 7일로 바꾸는 의도였는지 확인 필요. **확인 전까지 정책값과 UI
   문구를 확정하지 않는다**(2차 리뷰 확인 필요 항목과 동일 판단).
2. **R-0001 제약과 A-0001 계약의 모순(별건, 사용자 판단 필요)** — 2차 리뷰 B-1. R-0001 제약은
