...
**실패 이력**: 메일도 문서와 동일한 `index_failure.json`을 사용하므로 [확인 필요한 문서] 화면에서 `parse` 단계의 손상 메일을 확인하고 재시도·경로 제외할 수 있다. 기존 `mail_index_failure.json`이 있으면 공용 파일에 최신 기록을 병합해 원자 저장한 뒤 `mail_index_failure.migrated.json`으로 이름만 바꿔 보관한다. 병합 저장·보관 이동 중 하나라도 실패하면 원본은 그대로 유지한다.
...
`collector.exclude_files`는 문서와 메일의 공용 제외 목록이다. 메일 제외는 `mail_uid` 전체가 아니라
`source_file` 경로 단위로 적용한다. 같은 UID의 현재 DB 행이 제외 경로에서 만들어졌다면 그 행의 정확한
`chunk_id`를 `mail_scan_state.json.pending_deletes`에 먼저 원자 저장한 뒤 삭제하고, 같은 UID 복사본의
성공 캐시는 모두 무효화해 제외되지 않은 복사본이 다음 스캔에서 새 ID로 복구하게 한다. 제외 경로가 단순
복사본이고 현재 행의 출처가 다른 경로라면 그 복사본 캐시만 제거하고 현재 행은 유지한다.

삭제 실패 ID는 다음 사이클 후보 수집 전에 재시도하며, 남아 있는 ID는 `get_index_state()`와
`has_current_mail_uid()`의 현재성 판정에서 제외한다. 제외 정리는 `mail.enabled`와 무관하게 실행하고,
Retriever도 현재 제외 경로를 DB 조건과 공통 경로 키로 걸러 삭제 대기 중인 본문을 복호화·LLM 전달하지
않는다. 제외 해제는 실패 이력과 이미 예약된 과거 ID 삭제를 유지하며 다음 스캔에서 새 청크로 복구한다.

개별·일괄 제외는 같은 브리지 서비스를 사용한다. CollectorWorker 실행 중에는 저장·삭제 경합을 막기 위해
설정을 바꾸지 않고 `busy`를 반환한다. 경로 비교는 절대·실경로, 슬래시, Windows 대소문자를 통일하며,
구버전 DB 표기가 exact 조건과 다를 때도 `chunk_id/mail_uid/source_file` 세 열만 projection 조회해 보완한다.
